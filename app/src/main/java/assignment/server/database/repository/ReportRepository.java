...
public class ReportRepository {
  ...
  public static MonthlyAppointmentReport getMonthlyAppointmentReport(int year, int month)
    ...
    try (Connection conn = DatabaseManager.getConnection()) {
      ...
      try (PreparedStatement ps = conn.prepareStatement(topDocSql)) {
        ps.setDate(1, start);
        ps.setDate(2, end);
        try (ResultSet rs = ps.executeQuery()) {
          while (rs.next()) {
  ...
  public static PatientVisitSummaryReport getPatientVisitSummaryReport(int year, int month)
    ...
    try (Connection conn = DatabaseManager.getConnection()) {
      try (PreparedStatement ps = conn.prepareStatement(newPatientSql)) {
        ps.setTimestamp(1, start);
        ps.setTimestamp(2, end);
        try (ResultSet rs = ps.executeQuery()) {
          if (rs.next()) {
      ...
      try (PreparedStatement ps = conn.prepareStatement(patientsSql)) {
        ps.setTimestamp(1, start);
        ps.setTimestamp(2, end);
        try (ResultSet rs = ps.executeQuery()) {
          while (rs.next()) {
